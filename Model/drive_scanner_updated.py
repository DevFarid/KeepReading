...
from models import CModel, KNearest
from preprocessing import *
from data_representation_abstracted_copy1 import TrainingRepresentation, BWHistogram
...
def load_models():
    ...
    k_model = KNearest()
    k_model.train("trained_means.yaml")
...
def predict(model: CModel, data_rep: TrainingRepresentation, parameters: dict):
    prediction = model.predict(image, data_rep, parameters)
    return prediction

# getPID
def getPID(text: list, im=image, rotation=0):
    ...
    if truth:
        ...
        received, answer = getPID(pytesseract.image_to_data(np.rot90(im))['text'], np.rot90(im), rotation + 1)
...
# getMOD
def getMOD(drive_types: list, text: list):
    ...
    model_params = load_models()[0]
    model_drive = predict(model_params[0], BWHistogram(), model_params[1])
