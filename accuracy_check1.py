...
from data_representation_abstracted_copy1 import BWHistogram
...
if args['file'] is None:
    ...
    parameters['optimal_size'] = dataArr[0][1].shape[1]
    parameters['K'] = 1
...
def process_image(image):
    results = pytesseract.image_to_string(Preprocess.remove_bg_to_bw_blur_binary(image), output_type=Output.DICT)
    return results.copy()
...
k_nearest.train("trained_means.yaml")
...
def image_processing_worker():
    while True:
        ...
        if args["barcode_removal"]:
            ...
            #[PID, OCR read data, image]
            results = [thing[0], process_image(image), image, k_nearest.predict(image, BWHistogram(), parameters)]
        r_queue.put(results)
        d_queue.task_done()
        print("PID: ", thing[0])
...
for entry in processed_results:
    ...
    if ser_model_dict[entry[0]]['Model'].upper() in entry[3]:
        models[0].append(entry)
    else:
        print(ser_model_dict[entry[0]]['Model'])
        print(entry[3])
